...
const TechnicalInput = ({ label, value, unit, onChange, className = "" }: any) => (
    <div className={`relative group ${className}`}>
        ...
        <div className="flex items-center glass-surface-light border border-white/5 rounded-none overflow-hidden group-focus-within:ring-2 group-focus-within:ring-primary/40 group-focus-within:border-primary transition-all p-0.5 relative light-sweep">
            ...
            />
            {unit && <span className="glass-surface text-[9px] font-black text-txt-muted px-2.5 py-2 rounded-none border border-white/5 select-none z-10 min-w-[40px] text-center">{unit}</span>}
        </div>
    </div>
);
...
const PipeConfigCard = ({ title, pipe, catalog, onSelect, bottomMD, onDepthChange, onRoughnessChange, colorClass, icon: Icon }: {
    ...
    return (
        <div className="glass-surface rounded-none border border-white/5 shadow-xl p-4 relative overflow-hidden group hover:border-primary/30 hover:shadow-glow-primary transition-all h-full flex flex-col justify-between light-sweep">
            ...
            <div className="flex justify-between items-center mb-4 pl-1 relative z-10">
                <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-none glass-surface-light border border-white/10 ${colorClass.replace('bg-', 'text-')} shadow-glow-primary group-hover:scale-110 transition-transform duration-500`}>
                        <Icon className="w-5 h-5" />
                    </div>
                    <div><h3 className="text-sm font-black text-txt-main uppercase tracking-widest">{title}</h3></div>
                </div>
                <div className="text-[10px] font-mono font-black text-txt-muted bg-canvas px-2 py-0.5 rounded-none border border-white/5">{pipe.od}" OD</div>
            </div>
            <div className="pl-1 space-y-1.5 relative z-10 flex-1 flex flex-col justify-center">
                <div className="space-y-0.5">
                    ...
                    <select className="w-full bg-canvas border border-white/5 text-[11px] font-bold text-txt-main rounded-none p-2 outline-none focus:border-primary cursor-pointer" value={pipe.description} onChange={onSelect}>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                        <div className="bg-canvas border border-white/5 rounded-none p-1 text-center flex flex-col justify-center shadow-inner">
        ...
        <div className="flex flex-col gap-4 pb-8 animate-fadeIn">
            <div className="grid grid-cols-12 gap-4 shrink-0 transition-all duration-500">
                ...
                <div className="col-span-12 md:col-span-12 lg:col-span-4 transition-all hover:scale-[1.01]" style={{ animationDelay: '0.3s' }}>
                    <div className="glass-surface rounded-none-[2rem] border border-white/5 shadow-2xl p-6 flex flex-col justify-between relative overflow-hidden group hover:border-primary/40 transition-all duration-700 light-sweep h-full min-h-[280px]">
                        ...
                        <div className="flex justify-between items-center mb-3 pl-2 relative z-10">
                            <div className="flex items-center gap-3">
                                <div className="p-2 rounded-none bg-canvas border border-white/5 text-primary shadow-inner group-hover:shadow-[0_0_15px_rgb(var(--color-primary)/0.3)] transition-shadow"><Thermometer className="w-5 h-5" /></div>
                                <h3 className="text-sm font-black text-txt-main uppercase tracking-widest leading-none shadow-sm">{t('p1.env')}</h3>
                            </div>
                        </div>
                        <div className="space-y-4 relative z-10 flex-1 flex flex-col justify-center">
                            <div className="flex items-center gap-4 bg-canvas/60 p-4 rounded-none border border-white/5 backdrop-blur-sm shadow-inner group/temp">
                                ...
                                <span className="flex-1 h-1 bg-gradient-to-r from-secondary to-primary opacity-30 rounded-none"></span>
            ...
            <div className="grid grid-cols-12 gap-4 min-h-[500px]">
                ...
                <div className="col-span-12 lg:col-span-4 flex flex-col min-h-[500px] animate-fadeIn" style={{ animationDelay: '0.5s' }}>
                    <div className="glass-surface rounded-none-[2rem] border border-white/5 shadow-2xl flex-1 flex flex-col overflow-hidden relative">

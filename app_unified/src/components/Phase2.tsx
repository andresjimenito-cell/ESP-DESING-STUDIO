...
const TechInput = ({ label, value, unit, onChange, step = "any", disabled = false, className = "" }: any) => (
    <div className={`relative group ${disabled ? 'opacity-30 pointer-events-none grayscale' : ''} ${className}`}>
        ...
        <div className="flex items-center glass-surface-light border border-white/5 rounded-none overflow-hidden group-focus-within:ring-2 group-focus-within:ring-primary/40 transition-all p-0.5 relative light-sweep">
            <input
                type="number"
                step={step}
                value={value}
            ...
            />
            {unit && <span className="glass-surface text-[9px] font-black text-txt-muted px-2.5 py-1 rounded-none border border-white/5 select-none min-w-[40px] text-center z-10">{unit}</span>}
        </div>
    </div>
);

const CorrelationSelect = ({ label, value, onChange, options }: { label: string, value: string, onChange: (e: any) => void, options: string[] }) => (
    <div className="grid grid-cols-2 items-center gap-2 py-1.5 border-b border-white/5 last:border-0 hover:bg-surface-light/30 px-2 rounded-none transition-colors group">
        <label className="text-[10px] font-bold text-txt-muted uppercase tracking-wide group-hover:text-txt-main">{label}</label>
        <div className="relative">
            <select
                value={value}
                onChange={onChange}
                className="w-full bg-canvas text-[10px] font-black text-txt-main border border-white/5 rounded-none py-1 pl-2 pr-6 outline-none focus:border-primary appearance-none cursor-pointer"
            >
...
export const Phase2: React.FC<Props> = ({ params, setParams }) => {
    ...
    return (
        <div className="flex flex-col gap-4 pb-4 h-full overflow-hidden">
            ...
            {/* SMART TIPS BANNER */}
            {smartTips.length > 0 && (
                <div className="glass-surface border border-primary/30 rounded-none p-4 flex items-center gap-4 animate-fadeIn shrink-0 shadow-glow-primary/10">
                    <div className="p-2.5 bg-primary/20 rounded-none text-primary shrink-0 shadow-glow-primary animate-pulse"><Lightbulb className="w-5 h-5" /></div>
                    <div className="flex flex-wrap gap-x-8 gap-y-1">
                        <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mr-2">PVT Smart Intelligence:</h4>
                        {smartTips.map((tip, i) => (
                            <p key={i} className="text-[11px] font-bold text-txt-muted flex items-center gap-2.5"><span className="w-1.5 h-1.5 bg-primary rounded-none shadow-glow-primary"></span>{tip}</p>
                        ))}
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 flex-1 min-h-0 overflow-y-auto custom-scrollbar">
                ...
                <div className="glass-surface rounded-none-[2rem] border border-white/5 shadow-xl p-6 flex flex-col relative overflow-hidden group hover:border-primary/40 transition-all duration-700 animate-fadeIn h-full light-sweep" style={{ animationDelay: '0.1s' }}>
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10 shrink-0">
                        <div className="flex items-center gap-4">
                            <div className="p-3 rounded-none glass-surface-light text-primary border border-white/10 shadow-glow-primary group-hover:scale-110 transition-transform duration-500"><Flame className="w-6 h-6" /></div>
                            <div>
                                <h3 className="text-sm font-black text-txt-main uppercase tracking-[0.25em] leading-tight">{t('p2.oilPhase')}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="w-2 h-2 rounded-none bg-primary animate-pulse shadow-glow-primary"></span>
                    ...
                    <div className="space-y-4 relative z-10 flex-1 flex flex-col">
                        ...
                        <div className="glass-surface-light p-1 rounded-none flex shadow-inner border border-white/5 backdrop-blur-sm">
                            <button onClick={() => setParams({ ...params, fluids: { ...params.fluids, isDeadOil: false } })} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-none transition-all ${!params.fluids.isDeadOil ? 'bg-primary text-white shadow-glow-primary' : 'text-txt-muted hover:text-primary'}`}>{t('p2.sat')}</button>
                            <button onClick={() => setParams({ ...params, fluids: { ...params.fluids, isDeadOil: true } })} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-none transition-all ${params.fluids.isDeadOil ? 'bg-primary text-white shadow-glow-primary' : 'text-txt-muted hover:text-primary'}`}>{t('p2.dead')}</button>
                        </div>

                        <div className="glass-surface-light rounded-none-[2rem] p-5 border border-white/5 space-y-4 shadow-inner">
                        ...
                        {/* Density widget */}
                        <div className="mt-auto p-4 bg-primary/5 rounded-none border border-primary/15 shadow-inner group/density relative overflow-hidden">
                            ...
                            <div className="space-y-3 relative z-10">
                                <div className="h-2.5 w-full bg-surface-light/50 rounded-none overflow-hidden shadow-inner flex p-0.5 border border-white/5">
                                    <div className="h-full bg-gradient-to-r from-primary to-primary/50 rounded-none transition-all duration-1000 ease-out shadow-primary/20 relative"
                ...
                <div className="glass-surface rounded-none-[2rem] border border-white/5 shadow-xl p-6 flex flex-col relative overflow-hidden group hover:border-secondary/40 transition-all duration-700 animate-fadeIn h-full light-sweep" style={{ animationDelay: '0.2s' }}>
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10 shrink-0">
                        <div className="flex items-center gap-4">
                            <div className="p-3 rounded-none glass-surface-light text-secondary border border-white/10 shadow-glow-secondary group-hover:scale-110 transition-transform duration-500"><Waves className="w-6 h-6" /></div>
                            <div>
                                <h3 className="text-sm font-black text-txt-main uppercase tracking-[0.25em] leading-tight">{t('p2.waterPhase')}</h3>
                                <div className="flex items-center gap-2 mt-1 opacity-70">
                                    <span className="w-2 h-2 rounded-none bg-secondary animate-pulse shadow-glow-secondary"></span>
                    ...
                    <div className="space-y-4 relative z-10 flex-1 flex flex-col">
                        {/* Salinity Control */}
                        <div className="bg-secondary/5 p-3 xl:p-4 rounded-none-[2rem] border border-secondary/15 backdrop-blur-md relative overflow-hidden flex flex-col gap-2 shadow-xl">
                            <div className="flex items-center justify-between relative z-10 border-b border-secondary/10 pb-3">
                                ...
                                <span className="text-[8px] font-black bg-secondary/10 text-secondary px-2.5 py-1 rounded-none border border-secondary/20 uppercase tracking-tighter">Analytical Mode</span>
                            </div>
                            ...
                            <div className="space-y-4 relative z-10">
                                ...
                                <div className="flex items-center justify-center py-1">
                                    <div className="h-[1px] bg-gradient-to-r from-transparent via-secondary/30 to-transparent w-full relative">
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-surface px-3 text-[7px] font-black text-secondary uppercase tracking-[0.3em] backdrop-blur-sm border border-secondary/20 rounded-none">{t('p2.synced')}</div>
                                    </div>
                                </div>
                                <TechInput
                                    label={t('p2.salinity')}
                        ...
                        <div className="mt-auto p-4 bg-secondary/5 rounded-none border border-secondary/15 shadow-inner group/indicator relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between relative z-10 mb-3">
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-none bg-secondary animate-ping"></div>
                            ...
                            <div className="px-4 py-3 rounded-none border-t border-white/5 transition-all duration-500 flex items-center justify-between bg-secondary/10 text-txt-main">
                ...
                {/* 3. FLUID MIXTURE & GOR */}
                <div className="glass-surface rounded-none-[2rem] border border-white/5 shadow-2xl p-6 flex flex-col justify-between relative overflow-hidden group hover:border-primary/40 transition-all duration-700 animate-fadeIn h-full light-sweep" style={{ animationDelay: '0.3s' }}>
                    ...
                    <div className="flex items-center gap-4 mb-4 relative z-10 shrink-0">
                        <div className="p-3 rounded-none glass-surface-light text-primary border border-white/10 shadow-glow-primary group-hover:scale-110 transition-transform duration-500"><Droplets className="w-6 h-6" /></div>
                        <div>
                    ...
                    <div className="glass-surface-light rounded-none-[2rem] p-6 border border-white/5 flex flex-col items-center justify-center relative shadow-inner flex-1 mb-4">
                        ...
                        <input type="range" min="0" max="100" step="0.1" value={params.fluids.waterCut} onChange={e => handleWaterCutChange(parseFloat(e.target.value))} className="w-full h-1.5 bg-surface-light/50 rounded-none appearance-none cursor-pointer mt-5 accent-primary hover:accent-orange-500 transition-all" />
                    </div>

                    {/* Gradient Display */}
                    <div className="bg-primary/5 rounded-none p-4 border border-primary/10 flex justify-between items-center mb-4 group/grad relative overflow-hidden">
                    ...
                    <div className="space-y-2 relative z-10">
                        <div className="relative">
                            ...
                            {params.fluids.pb > 0 && !params.fluids.isDeadOil && (
                                <div className="mt-2 flex items-center justify-between bg-canvas p-3 rounded-none border border-white/5 shadow-inner group/rs">
                                    ...
                                    <button onClick={applyCalculatedGor} className="bg-surface hover:bg-primary/20 hover:text-primary text-txt-muted p-2 rounded-none shadow-sm transition-all border border-white/5 active:scale-90" title="Apply Calculated Rs"><Calculator className="w-5 h-5" /></button>
                                </div>
                            )}
                        </div>
            ...
            <div className="flex gap-4 shrink-0">
                {/* 4. CONTAMINANTS CARD */}
                <div className="flex-1 glass-surface rounded-none border border-white/5 shadow-2xl relative overflow-hidden transition-all duration-500 ease-in-out">
                    ...
                    <div className="w-full flex items-center justify-between p-4 bg-surface-light/30">
                        <div className="flex items-center gap-3">
                            <div className="p-2.5 rounded-none bg-danger/10 text-danger border border-danger/20 shadow-sm"><Biohazard className="w-5 h-5" /></div>
                        ...
                        <div className="flex items-center gap-3">
                            {hasImpurities && (
                                <div className="bg-danger/10 text-danger border border-danger/20 px-2 py-0.5 rounded-none flex items-center gap-2 animate-pulse">
                ...
                <div className="flex-1 glass-surface rounded-none border border-white/5 shadow-2xl relative overflow-hidden transition-all duration-500 ease-in-out">
                    ...
                    <div className="w-full flex items-center justify-between p-4 bg-surface-light/30">
                        <div className="flex items-center gap-3">
                            <div className="p-2.5 rounded-none bg-primary/10 text-primary border border-primary/20 shadow-sm"><Settings className="w-5 h-5" /></div>

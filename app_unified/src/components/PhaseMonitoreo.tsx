        ...
        const loadAutoFiles = async () => {
            try {
                ...
                if (resDesigns && resDesigns.ok && mounted) {
                    ...
                    const resExcel = await fetch('/DATAS DE DISE�'O.xlsx');
                    if (resExcel.ok) {
        ...
        reader.onload = (event) => {
            ...
            try {
                ...
                if (rawTests.length > 0) {
                    ...
                    alert(`�?xito: Se cargaron ${rawTests.length} registros históricos para ${activeWell.name}.`);
                } else {
...
const FloatingAiPanel = ({ fleet, selectedWell, language, t }: { fleet: WellFleetItem[], selectedWell?: WellFleetItem, language: string, t: any }) => {
    ...
    useEffect(() => {
        ...
        if (!apiKey) {
            ...
            setMsgs([{ role: 'model', text: language === 'es' ? "�O Error: No se encontró la clave de API (GEMINI_API_KEY)." : "�O Error: No API Key found (GEMINI_API_KEY)." }]);
            return;
        }
        ...
        try {
            ...
            setMsgs([{ role: 'model', text: "�s�️ Antigravity en modo offline o error de conexión. (Revisa tu API Key)" }]);
        }

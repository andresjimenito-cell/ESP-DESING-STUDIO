...
import { RotateCw, Play, Pause, AlertTriangle, Shield, Compass } from 'lucide-react';
import {
...
const CustomTooltip = ({ active, payload, colorSurface, colorSurfaceLight, colorTextMuted, params, pumpTVD, perfsTVD }: any) => {
    ...
    const zoneColor = isCased ? '#38bdf8' : '#fbbf24';
    let equipment = null;
...
const DrillingRig = (props: any) => {
    ...
    const ry = yScale(0);
    return (
        <g transform={`translate(${rx - 25}, ${ry - 78})`} style={{ pointerEvents: 'none' }}>
            <path d="M 25 10 L 5 78 L 45 78 Z" fill="none" stroke={isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)"} strokeWidth={1} />
            <path d="M 25 10 L 25 78" stroke={isDark ? "rgba(255,255,255,0.25)" : "rgba(0,0,0,0.25)"} strokeWidth={1.2} />
            {[-12, 10, 32, 54].map(dy => (
                <line key={dy} x1={25 - (78 - dy) * 0.25} y1={dy} x2={25 + (78 - dy) * 0.25} y2={dy}
                    stroke={isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)"} strokeWidth={0.8} />
            ))}
...
export const TrajectoryPlot: React.FC<TrajectoryPlotProps> = ({ survey, params }) => {
    ...
    // 3D Color overlay
    const [colorOverlay3D, setColorOverlay3D] = useState<'structure' | 'inc' | 'dogleg'>('structure');
    ...
    }, [processedData, yaw, pitch, zoom, colorOverlay3D, hoveredPointIdx, params, showGridCage3D, showShadows3D, showGeologySlices, showRig3D, isDark]);
    ...
    // Overlay toggle pill
    const overlayOptions: { mode: 'structure' | 'inc' | 'dogleg'; label: string; color: string }[] = [
        { mode: 'structure', label: 'Casing / Open', color: '#38bdf8' },
        { mode: 'inc', label: 'Inclinación', color: '#10b981' },
        { mode: 'dogleg', label: 'DLS', color: '#f59e0b' },
    ];
    ...
    return (
        <div className="h-full flex flex-col glass-surface rounded-[2rem] border border-surface-light shadow-2xl overflow-hidden relative group select-none">
            ...
            {/* ── HEADER ── */}
            <div className="relative z-20 flex items-center justify-between px-6 py-3.5 border-b border-surface-light/25 bg-surface/50 backdrop-blur-xl shrink-0">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                        <Compass className="w-4 h-4 text-primary animate-spin-slow" />
                    </div>
                    <div>
                        <div className="text-[10px] font-black text-txt-main uppercase tracking-[0.22em]">
                            Estación de Geonavegación Unificada
                        </div>
                        <div className="text-[8px] font-bold text-txt-muted uppercase tracking-widest mt-0.5">
                            3D · Litológico · Inclinación · DLS — Vista Integrada
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                    <span className="text-[8px] font-black text-txt-muted uppercase tracking-widest mr-1">Overlay 3D:</span>
                ...
                <div className="relative overflow-hidden bg-canvas/15" style={{ gridColumn: '1', gridRow: '1 / 2' }}>
                    ...
                    <div className="absolute bottom-3 right-3 flex flex-col gap-1.5 bg-surface/55 backdrop-blur-md p-2.5 rounded-xl border border-white/5">
                        {[
                            ...
                            { key: 'shad', label: 'Sombras', val: showShadows3D, set: setShowShadows3D },

...
const DEFAULT_PUMP: EspPump = {
  ...
  stages: 0,
  housingCount: 1, 
  maxStagesPerBody: 100, // Default limit
  minRate: 0,
  bepRate: 0,
  maxRate: 0,
  maxEfficiency: 0,
                ...
                if (pHeaderIdx !== -1) {
                    ...
                    for(let i = pHeaderIdx + 1; i < pumpJson.length; i++) {
                        ...
                        headers.forEach((key, col) => {
                            ...
                            else if (key === 'frequency' || key === 'hz') p.nameplateFrequency = Number(val);
                            else if (['h0','h1','h2','h3','h4','h5','p0','p1','p2','p3','p4','p5'].includes(key)) {
                                p[key] = Number(val);
                                if (Number(val) !== 0) valid = true;
                            }
                        });
                        
                        // Sanity Check & Defaults for Critical Missing Data
                        if (p.model && valid) {
                            if (!p.stages || p.stages === 100) p.stages = 1;
                            if (!p.maxRate && p.minRate) p.maxRate = p.minRate * 2;
                            if (!p.bepRate) p.bepRate = (p.minRate + p.maxRate) / 2;
                            if (!p.maxEfficiency) p.maxEfficiency = 70; // Default efficiency if missing
                            if (!p.maxGraphRate) p.maxGraphRate = p.maxRate * 1.2;
                            if (!p.nameplateFrequency) p.nameplateFrequency = 60;
                            if (!p.maxStagesPerBody) p.maxStagesPerBody = 100; // Default limit
                            if (!p.housingCount) p.housingCount = Math.ceil(p.stages / p.maxStagesPerBody);
